...
public class OptionState : BaseMenuState
{
    ...
    public string audioSTR = "Audio On";
    public string audioSTR2 = "Audio Off";
    ...
    public override void onGUI ()
    {
        ...
        //draw out box		
        GUI.Box (GUIHelper.screenRect (offsetX, offsetY, Screen.width, Screen.height), "Option Menu");
        ...
        if (myCAM.audio.isPlaying) {
            Debug.Log ("Touched");
            if (addButton (GUIHelper.screenRect (offsetX + .05f, offsetY + .325f, .35f, .1f), audioSTR)) {
                myCAM.audio.Stop ();
            }
        } else {
            if (addButton (GUIHelper.screenRect (offsetX + .05f, offsetY + .325f, .35f, .1f), audioSTR2)) {
                myCAM.audio.Play ();
            }
