...
{   
	private static FadeCamera mInstance = null;
	
	private static FadeCamera instance
	{
		get
		{
			if( mInstance == null )
			{
				mInstance = GameObject.FindObjectOfType(typeof(FadeCamera)) as FadeCamera;
				
				if( mInstance == null )
				{
					mInstance = new GameObject("FadeCamera").AddComponent<FadeCamera>();
				}
			}
